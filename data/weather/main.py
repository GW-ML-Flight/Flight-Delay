...
def load_and_filter_weather_data(csv_path: str) -> pl.DataFrame:
    ...
    for col in available_cols:
        ...
        if missing_values:
            df_filtered = df_filtered.with_columns(
                pl.when(pl.col(col).is_in(missing_values)).then(None).otherwise(pl.col(col)).alias(col)
            )
    ...
    df_filtered = df_filtered.with_columns([
        pl.when(pl.col("wind_valid")).then(pl.col("wind_dir")).otherwise(None).alias("wind_dir"),
        pl.when(pl.col("wind_valid")).then(pl.col("wind_speed")).otherwise(None).alias("wind_speed"),
    ])
    ...
    df_filtered = df_filtered.drop([
        "WND", "wnd_parts", "wind_dir", "wind_rad", "wind_valid", "wind_speed", "wind_speed_q", "wind_dir_q", "wind_type"
    ])
...
def process_multiple_files(directory: str | Path, output_path: str | None | Path = None) -> pl.DataFrame:
    """
...
if __name__ == "__main__":
    ...
    df = process_multiple_files(directory=Path(__file__).parent, output_path=Path(__file__).parent / "combined_weather_data.parquet")
